...
NETWORK_REPORT_DIR=${NETWORK_REPORT_DIR:-"${RUNTIME_DIR}/network/${project_name}/${exp_name}_${NET_EXPERIMENT}"}
PHASE_CSV=${PHASE_CSV:-"${NETWORK_REPORT_DIR}/phases.csv"}
PHASE_SUMMARY=${PHASE_SUMMARY:-"${NETWORK_REPORT_DIR}/phase_summary.csv"}
# A quoted glob can include monitor CSVs from every node. Only samples overlapping
# this training run are used, so older files are harmless.
NETWORK_CSV_PATTERN=${NETWORK_CSV_PATTERN:-"/data/xgq/net_monitor_*.csv"}
REPORT_STEP=${REPORT_STEP:-"first"}
mkdir -p "${NETWORK_REPORT_DIR}"
...
TOTAL_WORKERS=$((NGPUS_PER_NODE * NNODES))
...
bash -x examples/quickstart/training/start_child_nodes.sh

# ray start --head \
#   --node-ip-address=10.100.13.5 \
#   --port=6379 \
#   --dashboard-host=0.0.0.0 \
#   --dashboard-port=8265 \
#   --num-gpus=$NUM_GPUS
...
PY
    ...
    echo "Current Ray GPUs: ${GPU_COUNT}/${TOTAL_WORKERS}"

    if [ "$GPU_COUNT" -ge "$TOTAL_WORKERS" ]; then
        echo "All GPUs ready."
        break
    fi
...
ray job submit --runtime-env $RUNTIME_ENV \
    -- env RAY_OVERRIDE_JOB_RUNTIME_ENV=1 \
        VERL_PHASE_LOG="${PHASE_CSV}" \
        VERL_NETWORK_CSV_PATTERN="${NETWORK_CSV_PATTERN}" \
        VERL_PHASE_SUMMARY="${PHASE_SUMMARY}" \
    ...
    actor_rollout_ref.rollout.agent.num_workers="${TOTAL_WORKERS}" \
...
python3 tools/plot_network_by_phase.py \
    --network-csv "${NETWORK_CSV_PATTERN}" \
    --phase-csv "${PHASE_CSV}" \
    --output-dir "${NETWORK_REPORT_DIR}" \
    --label "${NET_EXPERIMENT}" \
    --step "${REPORT_STEP}"
