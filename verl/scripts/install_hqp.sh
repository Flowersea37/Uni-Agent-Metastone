...
if [ $USE_SGLANG -eq 1 ]; then
    pip install "sglang[all]==0.5.2" --no-cache-dir && pip install torch-memory-saver --no-cache-dir -i https://mirrors.aliyun.com/pypi/simple/
fi
pip install --no-cache-dir "vllm==0.11.0" -i https://mirrors.aliyun.com/pypi/simple/

echo "2. install basic packages"
### 改，transformer 版本过新
# pip install "transformers[hf_xet]>=4.51.0" accelerate datasets peft hf-transfer \
pip install "transformers[hf_xet]==4.57.1" accelerate datasets peft hf-transfer \
    "numpy>=2.0.0" "pyarrow>=15.0.0" pandas "tensordict>=0.8.0,<=0.10.0,!=0.9.0" torchdata \
    ray[default] codetiming hydra-core pylatexenc qwen-vl-utils wandb dill pybind11 liger-kernel mathruler \
    pytest py-spy pre-commit ruff tensorboard  -i https://mirrors.aliyun.com/pypi/simple/
...
pip install "nvidia-ml-py>=12.560.30" "fastapi[standard]>=0.115.0" "optree>=0.13.0" "pydantic>=2.9" "grpcio>=1.62.1" -i https://mirrors.aliyun.com/pypi/simple/
...
pip install --no-cache-dir flashinfer-python==0.3.1 -i https://mirrors.aliyun.com/pypi/simple/
...
python -m pip install TransferQueue -i https://pypi.tuna.tsinghua.edu.cn/simple
