...
class TrainingWorker(Worker, DistProfilerExtension):
    ...
    def train_mini_batch(self, data: TensorDict) -> TensorDict:
        ...
        """
        # The memory metrics emitted by ``train_batch`` must describe this actor
        # update, not the highest allocation observed since the Ray worker was
        # created (which can include model initialization and rollout setup).
        ...
        # Report the worst-case card across all actor ranks. With tensor
        # parallelism only the model-parallel source returns ``output``, but a
        # different rank may have reached a slightly higher allocator peak.
        if device.is_available():
            device.synchronize()
            peak_memory = torch.tensor(
                [device.max_memory_allocated(), device.max_memory_reserved()],
                dtype=torch.float64,
                device=torch.device(self.device_name, device.current_device()),
            )
            torch.distributed.all_reduce(peak_memory, op=torch.distributed.ReduceOp.MAX)
            if output is not None:
                output_metrics = tu.get(output, "metrics")
                output_metrics["perf/max_memory_allocated_gb"] = [peak_memory[0].item() / (1024**3)]
                output_metrics["perf/max_memory_reserved_gb"] = [peak_memory[1].item() / (1024**3)]
        return output
