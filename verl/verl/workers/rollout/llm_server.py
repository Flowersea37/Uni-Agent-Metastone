...
class GlobalRequestLoadBalancer:
    ...
    def acquire_server(self, request_id: str) -> tuple[str, ray.actor.ActorHandle]:
        ...
        if len(candidates) == 1:
            ...
            # Agent sessions make many short, sequential model calls. Always
            # choosing the first idle replica pins new sessions to the first
            # TP group even when every replica is free between calls.
            server_id = candidates[self._tie_break_cursor % len(candidates)]
            self._tie_break_cursor += 1
