...
class vLLMColocateWorkerExtension:
    ...
    def get_rollout_memory_stats(self):
        """Return vLLM's own post-profile memory accounting for this GPU."""
        runner = self.model_runner
        snapshot = getattr(self, "init_snapshot", None)
        return {
            "rank": int(getattr(self, "rank", 0)),
            "model_weights_bytes": int(getattr(runner, "model_memory_usage", 0)),
            "kv_cache_available_bytes": int(getattr(self, "available_kv_cache_memory_bytes", 0)),
            "requested_memory_bytes": int(getattr(self, "requested_memory", 0)),
            "non_torch_memory_bytes": int(getattr(self, "non_torch_memory", 0)),
            "peak_activation_memory_bytes": int(getattr(self, "peak_activation_memory", 0)),
            "cudagraph_memory_estimate_bytes": int(getattr(self, "cudagraph_memory_estimate", 0)),
            "initial_total_memory_bytes": int(getattr(snapshot, "total_memory", 0)) if snapshot else 0,
            "initial_free_memory_bytes": int(getattr(snapshot, "free_memory", 0)) if snapshot else 0,
        }
