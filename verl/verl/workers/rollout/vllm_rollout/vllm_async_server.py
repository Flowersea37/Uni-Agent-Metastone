...
class vLLMHttpServer:
    ...
    async def run_server(self, args: argparse.Namespace):
        ...
        await self._record_engine_memory(engine_client, vllm_config)
    ...
    async def _record_engine_memory(self, engine_client, vllm_config) -> None:
        """Persist vLLM's measured weight/KV accounting once per replica."""
        output_value = self.config.get("engine_memory_log", None)
        if not output_value:
            return
        worker_stats = await engine_client.collective_rpc(method="get_rollout_memory_stats")
        cache = vllm_config.cache_config
        fields = [
            "timestamp", "replica_rank", "node_rank", "worker_rank",
            "rollout_quantization", "gpu_memory_utilization",
            "model_weights_bytes", "kv_cache_available_bytes",
            "requested_memory_bytes", "non_torch_memory_bytes",
            "peak_activation_memory_bytes", "cudagraph_memory_estimate_bytes",
            "initial_total_memory_bytes", "initial_free_memory_bytes",
            "kv_cache_blocks", "kv_cache_size_tokens", "kv_cache_max_concurrency",
        ]
        path = Path(output_value)
        path.parent.mkdir(parents=True, exist_ok=True)
        with path.open("a", newline="") as target:
            fcntl.flock(target.fileno(), fcntl.LOCK_EX)
            needs_header = target.tell() == 0
                ...
                writer.writerow(
                    {
                        ...
                        "node_rank": self.node_rank,
                        "worker_rank": stats.get("rank", ""),
                        "rollout_quantization": self.config.get("quantization", None) or "none",
                        "gpu_memory_utilization": self.config.gpu_memory_utilization,
                        **{key: stats.get(key, 0) for key in fields if key in stats},
                        "kv_cache_blocks": cache.num_gpu_blocks or 0,
                        "kv_cache_size_tokens": getattr(cache, "kv_cache_size_tokens", 0) or 0,
                        "kv_cache_max_concurrency": getattr(cache, "kv_cache_max_concurrency", 0) or 0,
                    }
                )
            target.flush()
    ...
    async def generate(
        ...
        with RLInsightLogger.trace_state("vllm_generate", state_lane_id=f"replica_{self.replica_rank}"):
            ...
            observed_first_token_at = None
            async for output in generator:
                final_res = output
                if observed_first_token_at is None and output.outputs and output.outputs[0].token_ids:
                    observed_first_token_at = time.time()
        ...
        self._record_request_metrics(
            ...
            observed_first_token_at=observed_first_token_at,
    ...
    def _record_request_metrics(
        self, *, request_id, prompt_tokens, output_tokens, started_at, finished_at, observed_first_token_at, metrics
    ) -> None:
        """Record server-only request latency, excluding agent/tool execution."""
        output_value = self.config.get("request_metrics_log", None)
        if not output_value:
            return
        ...
        arrival = metric("arrival_time")
        scheduled = metric("first_scheduled_time") or metric("scheduled_ts")
        queued = metric("queued_ts")
        first_token = metric("first_token_time") or metric("first_token_ts")
        finished = metric("finished_time") or metric("last_token_time") or metric("last_token_ts")
        e2e_s = finished_at - started_at
        # vLLM >= 0.17 exposes first_token_latency directly because arrival_time
        # is wall-clock while *_ts fields use a monotonic clock.
        ttft_s = metric("first_token_latency")
        if ttft_s == "" and arrival != "" and first_token != "":
            ttft_s = first_token - arrival
        if ttft_s == "" and observed_first_token_at is not None:
            ttft_s = observed_first_token_at - started_at
        queue_s = scheduled - queued if queued != "" and scheduled != "" else ""
        if queue_s == "" and arrival != "" and scheduled != "":
            queue_s = scheduled - arrival
        decode_s = finished - first_token if first_token != "" and finished != "" else ""
        if decode_s == "" and observed_first_token_at is not None:
            decode_s = finished_at - observed_first_token_at
        decode_tokens = max(output_tokens - 1, 0)
        fields = [
            "timestamp", "request_start", "request_end", "global_steps", "replica_rank", "request_id", "rollout_quantization",
            "prompt_tokens", "output_tokens", "server_e2e_s", "ttft_s", "queue_s", "decode_s",
            "decode_tokens_per_s", "model_forward_s", "model_execute_s", "scheduler_s",
        ]
        row = {
            "timestamp": f"{finished_at:.6f}", "request_start": f"{started_at:.6f}",
            "request_end": f"{finished_at:.6f}", "global_steps": self.global_steps,
            "replica_rank": self.replica_rank, "request_id": request_id,
            "rollout_quantization": self.config.get("quantization", None) or "none",
            "prompt_tokens": prompt_tokens, "output_tokens": output_tokens,
            "server_e2e_s": f"{e2e_s:.9f}",
            "ttft_s": "" if ttft_s == "" else f"{ttft_s:.9f}",
            "queue_s": "" if queue_s == "" else f"{queue_s:.9f}",
            "decode_s": "" if decode_s == "" else f"{decode_s:.9f}",
            "decode_tokens_per_s": "" if not decode_s else f"{decode_tokens / decode_s:.6f}",
            "model_forward_s": metric("model_forward_time"),
            "model_execute_s": metric("model_execute_time"),
            "scheduler_s": metric("scheduler_time"),
        }
        path = Path(output_value)
        path.parent.mkdir(parents=True, exist_ok=True)
        with path.open("a", newline="") as target:
    ...
    def _apply_quantization(self) -> tuple[Optional[str], dict]:
        ...
        if qat_config_dict.get("enable", False):
            ...
            if quantization == "fp8":
                ...
                # Multimodal/composite Hugging Face configs such as Qwen3.5
                # keep the language-model depth under ``text_config`` rather
                # than on the top-level config. Preserve the existing path for
                # ordinary text-only models.
                text_config = getattr(hf_config, "text_config", hf_config)
                # Conditional-generation wrappers nest the causal LM below
                # ``language_model``; text-only models use ``model`` directly.
                lm_prefix = "language_model.model" if text_config is not hf_config else "model"
                num_hidden_layers = getattr(text_config, "num_hidden_layers", None)
                if num_hidden_layers is None:
                    raise ValueError(
                        "FP8 rollout quantization requires num_hidden_layers in "
                        "hf_config or hf_config.text_config"
                    )
                for layer in range(num_hidden_layers):
                    all_mlp_gate_layers.append(f"{lm_prefix}.layers.{layer}.mlp.gate")

                # Qwen3.5 interleaves full attention with Gated DeltaNet
                # layers. DeltaNet contains 64-wide projections, which cannot
                # use the 128x128 block-scaled CUTLASS FP8 kernel. Keep only
                # those small recurrent-attention projections in BF16; the
                # regular attention and every dense MLP remain FP8.
                layer_types = getattr(text_config, "layer_types", None)
                if layer_types:
                    for layer, layer_type in enumerate(layer_types):
                        if layer_type == "linear_attention":
                            all_mlp_gate_layers.extend(
                                [
                                    f"{lm_prefix}.layers.{layer}.linear_attn",
                                    f"{lm_prefix}.layers.{layer}.linear_attn.conv1d",
                                    f"{lm_prefix}.layers.{layer}.linear_attn.in_proj_qkvz",
                                    f"{lm_prefix}.layers.{layer}.linear_attn.in_proj_ba",
                                    f"{lm_prefix}.layers.{layer}.linear_attn.out_proj",
                                ]
                            )
                        elif layer_type == "full_attention":
                            # Grouped-query attention packs narrow K/V shards
                            # into qkv_proj; with TP=1 they include a 64-wide
                            # block that the block-FP8 GEMM cannot execute.
                            all_mlp_gate_layers.extend(
                                [
                                    f"{lm_prefix}.layers.{layer}.self_attn",
                                    f"{lm_prefix}.layers.{layer}.self_attn.qkv_proj",
                                ]
                            )

                # Qwen3.5 is exposed as a multimodal conditional-generation
                # model even for text-only rollouts. vLLM consequently runs a
                # dummy vision profile at startup. Its ViT dimensions are not
                # supported by the block-scaled CUTLASS W8A8 kernel, so keep
                # the small vision tower in BF16 and quantize the language
                # model only. ``ignored_layers`` uses exact module prefixes.
                vision_config = getattr(hf_config, "vision_config", None)
                vision_depth = getattr(vision_config, "depth", 0) if vision_config is not None else 0
                if vision_depth:
                    all_mlp_gate_layers.extend(
                        [
                            name
                            ...
                            for name in (
                                f"visual.blocks.{layer}.attn.qkv",
                                f"visual.blocks.{layer}.attn.proj",
                                f"visual.blocks.{layer}.mlp.linear_fc1",
                                f"visual.blocks.{layer}.mlp.linear_fc2",
                            )
                        ]
                    )
                    all_mlp_gate_layers.extend(
