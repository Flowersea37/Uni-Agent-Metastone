...
def get_module_from_param_name(model, name: str):
    ...
    # MBridge exports Qwen3.5 conditional-generation names with a synthetic
    # leading ``model`` and without vLLM's inner causal-LM ``model`` wrapper.
    # Normalize only when the receiving vLLM model has that composite layout.
    if hasattr(model, "language_model"):
        if path_parts and path_parts[0] == "model" and not hasattr(model, "model"):
            path_parts.pop(0)
        if path_parts and path_parts[0] == "language_model":
            if len(path_parts) > 1 and path_parts[1] not in ("model", "lm_head"):
                path_parts.insert(1, "model")
        elif path_parts and path_parts[0] == "lm_head":
            path_parts.insert(0, "language_model")
