...
def _fused_linear_for_ppo_bwd(
    ...
    input_ids: torch.LongTensor,
    temperature: float = 1.0,
    dvocab_weights_out: Optional[torch.FloatTensor] = None,
    ...
    if dvocab_weights_out is None:
        dvocab_weights = dlogits.t() @ hidden_states
    else:
        # The full vocabulary gradient can be several GiB. Accumulate each
        # chunk into its existing buffer rather than materializing another
        # full-size matmul result at every backward chunk.
        dvocab_weights_out.addmm_(dlogits.t(), hidden_states)
        dvocab_weights = None
    ...
    def backward(ctx, dlog_probs: Optional[torch.FloatTensor], dentropy: Optional[torch.FloatTensor]):
        ...
        for chunk_start in range(0, T, chunk_size):
            ...
            h, v = _fused_linear_for_ppo_bwd(
                ...
                input_ids=input_ids[chunk_start:chunk_end],
                temperature=temperature,
                dvocab_weights_out=dvocab_weights,
            )

            if hidden_states.requires_grad:
                dhidden_states[chunk_start:chunk_end] += h
            if vocab_weights.requires_grad and v is not None:
                dvocab_weights += v
