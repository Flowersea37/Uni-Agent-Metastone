...
def make_megatron_module(
    ...
    if bridge is not None:
        ...
        if provider is not None:
            ...
            if (
                "cp_comm_type" not in inspect.signature(GatedDeltaNet.__init__).parameters
                and not getattr(GatedDeltaNet, "_verl_cp_compat_patched", False)
            ):
                _original_gdn_init = GatedDeltaNet.__init__

                def _gdn_init_compat(self, *args, cp_comm_type=None, **kwargs):
                    return _original_gdn_init(self, *args, **kwargs)

                GatedDeltaNet.__init__ = _gdn_init_compat
                GatedDeltaNet._verl_cp_compat_patched = True
