...
def patch_fsdp2_missing_unsharded_param() -> None:
    """Skip gradient conversion for FSDP2 parameters never unsharded in a step.

    PyTorch 2.11's ``to_accumulated_grad_if_needed`` reads ``_unsharded_param``
    even for parameters that did not participate in the current forward pass.
    Those parameters have no unsharded gradient to convert.
    """
    if version.parse(torch.__version__).release[:2] != (2, 11):
        return

    from torch.distributed.fsdp._fully_shard._fsdp_param import FSDPParam

    original = FSDPParam.to_accumulated_grad_if_needed
    if getattr(original, "_verl_missing_unsharded_param_guard", False):
        return
    ...
    guarded._verl_missing_unsharded_param_guard = True
...
@contextmanager
def _init_empty_weights_compatible_with_transformers():
    """Keep HF's loaded-parameter marker out of Accelerate's constructor kwargs.

    Accelerate copies a parameter's ``__dict__`` into ``Parameter.__new__`` when
    registering it on meta. Transformers 5 marks loaded parameters with
    ``_is_hf_initialized`` before registration, but ``Parameter.__new__`` does
    not accept that keyword. Preserve the marker on the final meta parameter.
    """
    ...
    with init_empty_weights():
        ...
        def register_without_hf_marker(module, name, param):
            if param is None or not hasattr(param, "_is_hf_initialized"):
                return register_parameter(module, name, param)
            marker = param._is_hf_initialized
            del param._is_hf_initialized
            try:
                return register_parameter(module, name, param)
            finally:
                param._is_hf_initialized = marker
                registered = module._parameters.get(name)
                if registered is not None:
                    registered._is_hf_initialized = marker

        nn.Module.register_parameter = register_without_hf_marker
        try:
            yield
        finally:
...
def get_init_weight_context_manager(use_meta_tensor=True, mesh: DeviceMesh = None):
    meta_init_weights = _init_empty_weights_compatible_with_transformers
    cpu_init_weights = lambda: torch.device("cpu")
    if use_meta_tensor:
        if mesh is None:
            init_context = meta_init_weights if torch.distributed.get_rank() != 0 else cpu_init_weights
        else:
            init_context = meta_init_weights if mesh.get_coordinate()[-1] != 0 else cpu_init_weights
    else:
