...
def run_ppo(config, task_runner_class) -> None:
    ...
    if not ray.is_initialized():
        ...
        if config.transfer_queue.enable:
            ...
            # Hydra's struct mode rejects new keys on its DictConfig. Copy to
            # a plain dict before adding the variable for Ray workers.
            runtime_env_vars = dict(runtime_env_kwargs.get("env_vars") or {})
    ...
    # Keep the trainer controller on the Ray Job's (head) node. Phase/network
    # report paths are node-local, and the post-job plotting command runs there.
    # GPU workers remain free to use every node in the resource pool.
    runner_options = {
    ...
    if (
        ...
        runner_options["runtime_env"] = {"nsight": nsight_options}
    runner = task_runner_class.options(**runner_options).remote()
