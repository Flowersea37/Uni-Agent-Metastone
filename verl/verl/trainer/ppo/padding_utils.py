...
def construct_minimal_padding_template(
    ...
    # Distillation fields span the full prompt + response. The copied source
    # fields are much longer than this two-token synthetic sample.
    for key in ("teacher_logprobs", "teacher_ids"):
        source = template_sample.get(key)
        if isinstance(source, torch.Tensor):
            template_sample[key] = torch.zeros(
                (input_ids.numel(), *source.shape[1:]), dtype=source.dtype, device=source.device
            )
