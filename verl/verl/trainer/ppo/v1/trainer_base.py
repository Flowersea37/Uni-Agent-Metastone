...
from verl.utils.phase_recorder import record_phase
...
class PPOTrainer(ABC):
    ...
    @contextmanager
    def _phase_timer(self, name: str, timing_raw: dict[str, float], **kwargs):
        """Keep veRL timing metrics and optionally emit a wall-clock phase interval."""
        step = getattr(self, "global_steps", None)
        trigger_step = getattr(self, "local_trigger_step", None)
        with record_phase(name, step=step, trigger_step=trigger_step):
    ...
    def _setup(self):
        ...
        trainer_ray_resource = os.environ.get("OPD_TRAINER_RAY_RESOURCE")
        if trainer_ray_resource:
            # Pin the four-GPU training pool to the labeled head node so the
            # eight-GPU worker remains available for standalone rollout.
            self.resource_pool_manager.resource_pool_dict["global_pool"].accelerator_type = trainer_ray_resource
        ...
        if self.use_teacher_policy:
            if os.environ.get("OPD_TEACHER_URL"):
                from uni_agent.framework.opd_external_teacher import ExternalTeacherModelManager

                self.teacher_model_manager = ExternalTeacherModelManager(self.config)
            else:
    ...
    def fit(self, agent_loop_manager: AgentLoopManager):
        ...
        while current_epoch < self.config.trainer.total_epochs and self.global_steps <= self.total_training_steps:
            ...
            with self._phase_timer("step", self.timing_raw):
                self.on_step_begin()
                ...
                if self.config.trainer.save_freq > 0 and (
                    ...
                    with self._phase_timer("save_checkpoint", self.timing_raw, color="green"):
            ...
            if self.config.trainer.test_freq > 0 and (
                ...
                with self._phase_timer("testing", self.timing_raw, color="green"):
                    self.on_validate_begin()
            ...
            append_gpu_memory_step(metrics, self.config, self.global_steps)
    ...
    def _step_once(self, metrics: dict, timing_raw: dict, sample_batch_size: int) -> KVBatchMeta:
        ...
        with self._phase_timer("gen", timing_raw, color="red"):
        ...
        if self.reward_loop_manager.reward_loop_worker_handles is None:
            with self._phase_timer("reward", timing_raw, color="yellow"):
    ...
    def _init_resource_pool_mgr(self):
        ...
        if is_distillation_enabled(distillation_config) and not os.environ.get("OPD_TEACHER_URL"):
    ...
    def _log_rollout_data(self, batch: KVBatchMeta, timing_raw: dict, rollout_data_dir: str):
        ...
        with self._phase_timer("dump_rollout_generations", timing_raw, color="green"):
