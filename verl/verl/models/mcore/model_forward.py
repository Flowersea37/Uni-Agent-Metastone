...
def model_forward_gen(vision_model: bool = False):
    def model_forward(
        ...
        # Extra 128-row padding is required by block-scaled recipes only.
        # Applying it to delayed scaling makes Qwen3.5 GDN's hidden-state
        # length disagree with the packed cu_seqlens metadata.
        block_padding = fp8 in ["e4m3", "hybrid"] and fp8_recipe in ["blockwise", "mxfp8"]
        low_precision = fp8 in ["e4m3", "hybrid"] or fp4 == "e2m1"
        use_fp8_padding = block_padding or (data_format == "bshd" and low_precision)
