...
def apply_fp8_dequantized_backward_patch():
    """Use TE's dequantized backward for the FP8 compatibility mode.

    Megatron's ``fp8_wgrad=False`` only disables FP8 wgrad.  Qwen3.5 TP8 can
    still enter an FP8 dgrad GEMM whose columnwise weight layout violates the
    cuBLASLt ``lda % 16`` requirement.  Transformer Engine exposes
    ``backward_override='dequantized'`` specifically for this fallback.  In
    contrast to ``high_precision``, it is compatible with DelayedScaling since
    it does not require saving the original BF16 input.

    The wrapper is intentionally inert unless FP8 is enabled *and*
    ``fp8_wgrad`` is false, so the default BF16 path returns the original
    recipe without mutation.
    """
    import megatron.core.fp8_utils as fp8_utils

    if getattr(fp8_utils.get_fp8_recipe, "_verl_fp8_dequantized_backward_patch", False):
        return

    original_get_fp8_recipe = fp8_utils.get_fp8_recipe

    def get_fp8_recipe_with_compatible_backward(config):
        recipe = original_get_fp8_recipe(config)
        if getattr(config, "fp8", None) in ("e4m3", "hybrid") and not getattr(config, "fp8_wgrad", True):
            recipe.backward_override = "dequantized"
        return recipe

    get_fp8_recipe_with_compatible_backward._verl_fp8_dequantized_backward_patch = True
    fp8_utils.get_fp8_recipe = get_fp8_recipe_with_compatible_backward
