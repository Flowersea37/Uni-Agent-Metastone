...
def preprocess_thd_engine(
    ...
    use_fp8_row_padding: bool = False,
    ...
    if use_fp8_padding:
        per_seq_align, total_align = _compute_fp8_thd_align_size(align_size)
        align_size = per_seq_align
    elif use_fp8_row_padding:
        # Transformer Engine FP8 GEMMs require the rank-local flattened row
        # count to be divisible by 8. Sequence parallel shards the global THD
        # rows over TP ranks, so align the global total to TP * CP * layout * 8.
        # Unlike block-scaled FP8 padding, individual sequences need no extra
        # 16/128 alignment for delayed scaling.
        total_align = align_size * 8
    ...
    if use_fp8_padding or use_fp8_row_padding:
        # Extend only the last sequence so packed metadata and hidden-state
        # length remain identical. Postprocessing uses the original nested
        # offsets and removes these synthetic rows.
        pad_size_last = (total_align - cu_seqlens_padded[-1] % total_align) % total_align
        cu_seqlens_padded[-1] += pad_size_last
        seqlens_in_batch_padded[-1] += pad_size_last
