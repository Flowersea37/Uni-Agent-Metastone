...
class TestLoadBalancerRouting:
    ...
    def test_sequential_requests_rotate_across_idle_servers(self, ray_for_lb):
        lb = GlobalRequestLoadBalancer.remote(servers={"s0": None, "s1": None, "s2": None, "s3": None})
        servers = []
        for i in range(4):
            server_id = ray.get(lb.acquire_server.remote(request_id=f"session-{i}"))[0]
            servers.append(server_id)
            ray.get(lb.release_server.remote(server_id=server_id))
        assert servers == ["s0", "s1", "s2", "s3"]
